...
use halo2_proofs::plonk::{create_proof, ProvingKey, VerifyingKey, verify_proof};
...
pub fn prove(inputs: HashMap<String, Vec<Fp>>, srs: &ParamsKZG<Bn256>, pk: &ProvingKey<G1Affine>) -> Result<(Vec<Fp>, Vec<u8>), String> {
    ...
    let a = Fp::from(1); // F[0]
    let b = Fp::from(1); // F[1]
    let out: Fp = inputs.get("out")
        .ok_or("`out` value not found in proof input".to_string())?
        .get(0)
        .ok_or("`out` value list is empty".to_string()).unwrap().clone();
...
pub fn verify(proof: Vec<u8>, inputs: &Vec<Fr>, srs: &ParamsKZG<Bn256>, vk: &VerifyingKey<G1Affine>) -> Result<bool, ()> {
